...
class Arachne(repair.Repair):
    ...
    def set_options(self, dataset, kwargs):
        ...
        if "num_run" in kwargs:
            self.num_run=int( kwargs['num_run'] )
    ...
    def _compute_gradient(self, model, input_neg, desc=True):
        ...
        sess = K.get_session()
        sess.as_default()
        loss = loss_func(input_neg[1], model.output)
        get_grad_kernel = K.gradients(loss, layer.kernel)[0]  # TODO bias?
        grad_kernel = sess.run(get_grad_kernel,
                               feed_dict={model.input: input_neg[0]})
    ...
    def _compute_forward_impact(self, model, input_neg, candidates, num_grad):
        ...
        sess = K.get_session()
        w = sess.run(target_layer.kernel)
    ...
    def optimize(self,
        ...
        if self.kaminari_depth != 0:  # if kaminari did something
            global_best_position = self.kaminari.de_translate_weights(global_best_position)
            weights = [[w[0]+self.kaminari_depth, w[1], w[2]] for w in weights]
            model = self.original_complete_model  # go back to complete model for output
    ...
    def _get_velocity_bounds(self, model):
        ...
        velocity_bounds = {}
        for layer_index in trange(1, len(model.layers),
        ...
        # The above for loop is skipped if model has a single layer
    ...
    def _criterion(self, model, location, input_pos, input_neg):
        ...
        if not self.risk_aware:  # use default Arachne
            ...
            if not self.normalized_fitness:
...
#<<<<<<< HEAD
#            loss_foo = tf.keras.losses.CategoricalCrossentropy(reduction=tf.keras.losses.Reduction.NONE)  # model.loss_functions[0]
#            # we want no reduction since we want to know the individual loss for each input
#            neg_loss = loss_foo(input_neg[1], neg_predictions).eval(session=sess)#.numpy()
#            pos_loss = loss_foo(input_pos[1], pos_predictions).eval(session=sess)#.numpy()
#=======
#>>>>>>> 89f60bab758dcc32b761e243d3ce4be26caba8a4
    ...
    def create_optimization_history(self, output_dir, number_of_weights, file_name=None):
        ...
        :param file_name: in case we want to have multiple logs, to avoid overwriting on future runs
        :return:
        """
        output_dir = Path(output_dir)
    ...
    def update_optimization_history(self, locations, output_dir, iteration, best_scores,
                                    current_scores, file_name=None):
        ...
        if file_name is None:
            file_name = 'optimization_history.csv'
        with open(output_dir.joinpath(file_name), 'a') as f:
            writer = csv.writer(f, delimiter='\t')
            writer.writerows(log_rows)

    def finish_optimization_history(self, t, particle_num, score, location, output_dir, file_name=None):
    ...
    def load_input_neg(self, dir):
        ...
        :return:
        """
        return self.dataset.load_repair_data(dir)
    ...
    def evaluate(self,
        ...
        for i in range(num_runs):
            ...
            repair_images, repair_labels = dataset.load_repair_data(
                target_data_dir
            )
            score = model.evaluate(repair_images,
                                   repair_labels,
                                   verbose=0,
                                   batch_size=self.batch_size)
            rr = score[1] * 100
