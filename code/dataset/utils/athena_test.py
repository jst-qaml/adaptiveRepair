...
from repair.athena.athena import Athena


def athena_test(dataset, kwargs):
    ...
    if data_dir.joinpath(r'athena_results.json').exists():
        with open(data_dir.joinpath(r'athena_results.json'),
                  'r') as f:
            results = {}
    ...
    for key in weights_candidates:
        if key in results:
            ...
            continue
        model_repaired = _load_and_mutate(dataset,
                                          model_dir,
                                          weights_candidates[key])
        labels = key.split('_')
        results[key] = {}

        for lab in labels:
            score = model_repaired.evaluate(negative_map[lab][0],
                                            negative_map[lab][1])
            results[key][lab] = {}
            results[key][lab]['loss'] = score[0]
            results[key][lab]['accuracy'] = score[1]

        score = model_repaired.evaluate(negative_all_images,
                                        negative_all_labels,
                                        verbose=0)
...
def _permutations_if_needed(weights_repaired, c):
    ...
    if len(permutations_set) != 0:
        ...
        return [e + tuple(non_permutations_set)
                for e in permutations_tuple]
    else:
        return [c]


def _load_weights_repaired(target_data_dir,
                           athena,
                           skip_repaired_value=False):
    ...
    for label in weights:
        ...
        if 'repair_priority' not in weights[label]:
            ...
            continue
        elif not weights[label]['repair_priority']:
            continue
        if 'repaired_values' in weights[label] or skip_repaired_value:
            ...
            for _weight in weights[label]['weights']:
                ...
                if skip_repaired_value:
                    ...
                    val = np.float32(weights[label]['repaired_values']
                                     [idx])
                    idx += 1
        ...
        if p.is_dir():
            if p.joinpath(r'repair.h5').exists():
                test_images, test_labels = \
                    test._load_dataset(p, r'repair.h5')
                negative_map[p.name] = [test_images, test_labels]
    return negative_map


def _load_and_mutate(dataset, model_dir, weights):
