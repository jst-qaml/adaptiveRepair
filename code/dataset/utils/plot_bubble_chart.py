...
def plot_bubble_chart(ground_truth, pred_labels, output_path):
    ...
    labels = list(map(lambda l: str(l), np.unique(ground_truth)))
    cm = confusion_matrix(ground_truth, pred_labels, normalize='true')
    df = pd \
        .DataFrame(cm) \
        .set_axis(labels, axis='columns') \
        .set_axis(labels, axis='index')
...
def plot_each_bubble_chart(fig,
                           ground_truth,
                           pred_labels,
                           target=None,
                           marker='o'):
    ...
    if target is not None:
        ...
        labels = list(map(lambda l: str(l), np.unique(ground_truth)))
    preds = list(map(lambda l: str(l), np.unique(pred_labels)))
    cm = confusion_matrix(ground_truth, pred_labels, normalize='true')
    df = pd \
        .DataFrame(cm) \
        .set_axis(preds, axis='columns') \
        .set_axis(preds, axis='index')
    x, y = np.meshgrid(df.columns,
                       df.filter(items=labels, axis='index').index)
    ax = fig.add_subplot()

    ax.scatter(x=x.flatten(),
               y=y.flatten(),
