...
class VisionTransferModel(model.Model):
    ...
    def set_extra_config(self, *kwargs):
        ...
        kwargs = kwargs[0]['kwargs']
        pre_train_dir = Path(kwargs['data_dir'])
        train_path = pre_train_dir.joinpath(r'train.h5')
        self.x_train = np.array(h5py.File(train_path, 'r')['images'])
        self.y_train = np.array(h5py.File(train_path, 'r')['labels'])
    ...
    def create_vit_classifier(self, data_augmentation, input_shape):
        ...
        # Encode patches.
        encoded_patches = PatchEncoder(self.num_patches,
